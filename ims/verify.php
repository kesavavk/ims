...
while($record = mysql_fetch_array($recordset)){
	...
	$_SESSION["fname"] = $record["fname"];	
			...
			header("Location:admin/backg.php?uname=".$record["uname"]);
		    exit;
			}else if($record["utype"] == 2){
				header("Location:prm/backg.php?uname=".$record["uname"]);
			}
			else if($record["utype"] == 0){
				header("Location:stm/backg.php?uname=".$record["uname"]);
			}
			else{
